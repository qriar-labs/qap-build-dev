package main
  ...
  func requestToken(grantType string, refreshToken string) (string, string, time.Time, error) {
    ...
    if grantType == "password" {
      form = fmt.Sprintf("grant_type=password&client_id=&client_secret=&username=&password=")
    } else {
      form = fmt.Sprintf("grant_type=refresh_token&client_id=&client_secret=&refresh_token=%s", refreshToken)
    }
