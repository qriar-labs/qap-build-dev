package main
  ...
  func requestToken(grantType string, refreshToken string) (string, string, time.Time, error) {
    ...
    if grantType == "password" {
      form = fmt.Sprintf("grant_type=password&client_id=qap-front&client_secret=tIk6IvluTeOS2o23C4nffL48j2fyF50H&username=admin@konneqt.io&password=LZNK1kguf_KI")
    } else {
      form = fmt.Sprintf("grant_type=refresh_token&client_id=qap-front&client_secret=tIk6IvluTeOS2o23C4nffL48j2fyF50H&refresh_token=%s", refreshToken)
    }
  
    req, err := http.NewRequest("POST", "https://idp.konneqt.cloud/realms/qap-dev/protocol/openid-connect/token", strings.NewReader(form))
    if err != nil {
