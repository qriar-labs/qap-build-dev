package main
  ...
  func requestToken(grantType string, refreshToken string) (string, string, time.Time, error) {
    ...
    if grantType == "password" {
      form = fmt.Sprintf("grant_type=password&client_id=undefined&client_secret=undefined&username=undefined&password=undefined")
    } else {
      form = fmt.Sprintf("grant_type=refresh_token&client_id=undefined&client_secret=undefined&refresh_token=%s", refreshToken)
    }
  
    req, err := http.NewRequest("POST", "undefined", strings.NewReader(form))
    if err != nil {
